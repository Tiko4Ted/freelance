...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    const input = adminStatusSchema.parse(body);
    const application = await AdminApplicationService.updateStatus(id, input);

    return NextResponse.json({ application });
  } catch (error) {
    if (error instanceof ZodError) {
      return NextResponse.json(
        { error: "Invalid status input", issues: error.flatten() },
        { status: 400 },
      );
    }
